...
use crate::types::{
    AuditEntry, ConnectionStats, ConsistencyMode, PaginatedTuples, PaginationCursor, PaginationParams,
    PartitionId, Relation, RelationshipTuple, ResourceId, Revision, RevisionToken, SubjectId,
    TupleKey, TupleMutation,
};
    ...
    fn wal_size_mb(&self) -> Option<f64> {
        self.runtime.block_on(async {
            ...
            let row: mysql_async::Row = conn
                .exec_first(
                    "SELECT ROUND(@@innodb_log_file_size / 1048576, 2)",
                    (),
                )
                .await
                .ok()??;
