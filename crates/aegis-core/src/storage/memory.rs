...
impl StorageBackend for InMemoryStorage {
    ...
    fn query_tuples(
        ...
        let mut result: Vec<RelationshipTuple> = inner
            ...
            .values()
            .filter(|t| {
                if let Some(ref st) = filter.subject_type && !t.subject.as_str().starts_with(st.trim_end_matches('#')) {
                    return false;
                }
                if let Some(ref rel) = filter.relation && t.relation != *rel {
                    return false;
                }
                if let Some(ref ot) = filter.object_type && !t.object.as_str().starts_with(ot) {
                    return false;
                }
                true
            })
    ...
    fn verify_audit_chain(&self, partition_id: &PartitionId) -> AegisResult<Option<String>> {
        ...
        for (i, event) in inner.events.iter().enumerate() {
            let (stored_prev_hash, stored_event_hash) = inner
                .event_hashes
                .get(i)
                .ok_or_else(|| AegisError::StorageCorruption(format!(
                    "missing hash chain entry for event {} (revision {})",
                    i, event.revision.as_u64()
                )))?;

            if *stored_prev_hash != last_event_hash {
                return Ok(Some(format!(
                    "Chain break at event {} (revision {}): expected previous_hash='{}', got '{}'",
                    i, event.revision.as_u64(), last_event_hash, stored_prev_hash
                )));
            }
            ...
            let metadata_str = event.metadata.as_ref()
            ...
            if expected != *stored_event_hash {
                return Ok(Some(format!(
                    ...
                    i, event.revision.as_u64(), expected, stored_event_hash
                )));
            }
