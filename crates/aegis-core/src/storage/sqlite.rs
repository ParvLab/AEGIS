...
impl StorageBackend for SqliteStorage {
    ...
    fn close(&self) -> AegisResult<()> {
        if self.config.wal_mode && self.config.path != ":memory:" && let Ok(conn) = self.pool.get() {
    ...
    fn recover_from_events_impl(
        ...
        self.with_write_tx(|conn| {
            ...
            for row in rows {
                ...
                if let Some(target) = to_revision && rev > target {
                    continue;
                }
                let now = Utc::now().to_rfc3339();
...
impl StorageTransaction for SqliteTransaction {
    ...
    fn rollback(mut self: Box<Self>) -> AegisResult<()> {
        if !self.committed && let Some(conn) = self.conn.take() {
