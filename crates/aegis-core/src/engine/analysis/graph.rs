...
impl GraphEngine {
    ...
    pub fn reachable_subjects(
        ...
        if let Some(ttl) = cache_ttl_ms && let Some(cached) = self.get_cached_analysis(&cache_key, ttl) {
            return Ok(cached);
        }
    ...
    fn set_cached_analysis(&self, key: &str, value: &impl serde::Serialize, ttl_ms: u64) {
        if let Ok(mut cache) = self.analysis_cache.lock() && let Ok(json) = serde_json::to_string(value) {
