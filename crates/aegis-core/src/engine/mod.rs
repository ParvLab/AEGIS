...
impl GraphEngine {
    ...
    fn emit_log(&self, level: hooks::LogLevel, message: &str, context: &str) {
        if let Ok(guard) = self.logger.lock() && let Some(ref logger) = *guard {
    ...
    pub fn stop_watcher(&self) {
        ...
        if let Ok(mut guard) = self.watcher_thread.lock() && let Some(handle) = guard.take() {
            handle.join().ok();
        }
    }
        ...
        std::thread::scope(|s| {
            for rel_name in &rel_names {
                ...
                let _handle = s.spawn(move || {
                    ...
                    if let Ok(r) = result && r.found && evaluate_condition_if_present(cond.as_ref(), ctx_ref.as_ref())
                    {
            ...
            if let Some(type_def) = schema.types.get(&resource_type) {
                'deny_outer: for deny_def in &type_def.deny {
                    for deny_rel in &deny_def.relations {
                        ...
                        if let Ok(tr) = traversal_result && tr.found {
                            allowed = false;
                            break 'deny_outer;
                        }
                    }
    ...
    fn maybe_checkpoint_wal(&self) {
        ...
        if let Some(wal_size) = self.storage.wal_size_mb() && wal_size > threshold {
